...
export function ResultsDisplay({ result }: { result: GradeResult }) {
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      {/* Hero grade */}
      <div className="text-center space-y-5 py-8 bg-white rounded-2xl border border-border">
        <div className="flex justify-center">
          <GradeBadge grade={result.grade} color={result.grade_color} size="xl" />
        </div>
        <div>
          <div className="font-display text-6xl text-text">{result.total_score}</div>
          <div className="text-text-dim text-sm mt-1">out of 100</div>
        </div>
        <p className="text-lg text-text-secondary max-w-lg mx-auto leading-relaxed px-6">
          {result.summary}
        </p>
        <span
      ...
      <div className="rounded-2xl border border-border bg-white p-6 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="font-display text-xl text-text">Score Breakdown</h2>
          <span className="text-[10px] text-text-dim uppercase tracking-widest">Tap to learn more</span>
        </div>
      ...
      {result.recognized_ingredients.length > 0 && (
        <div className="rounded-2xl border border-border bg-white p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-display text-xl text-text">Ingredient Analysis</h2>
            <span className="text-[10px] text-text-dim uppercase tracking-widest">
              {result.recognized_ingredients.length} ingredients scored
            </span>
          </div>
      ...
      {result.synergies.length > 0 && (
        <div className="rounded-2xl border border-green-200 bg-green-50 p-6">
          <h2 className="font-display text-xl text-green-800 mb-1">Synergy Bonuses</h2>
          ...
          <div className="space-y-3">
            {result.synergies.map((syn, i) => (
              <div key={i} className="flex items-start gap-3 rounded-xl bg-white border border-green-200 p-3">
                <div className="w-6 h-6 rounded-full bg-green-100 flex items-center justify-center shrink-0 mt-0.5">
                  <svg className="w-3.5 h-3.5 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
                  </svg>
                </div>
                <span className="text-sm text-text-secondary">{syn}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {result.red_flags.length > 0 && (
        <div className="rounded-2xl border border-red-200 bg-red-50 p-6">
          <h2 className="font-display text-xl text-red-800 mb-1">Red Flags</h2>
          <p className="text-sm text-text-secondary mb-4">
          ...
          <div className="space-y-3">
            {result.red_flags.map((flag, i) => (
              <div key={i} className="flex items-start gap-3 rounded-xl bg-white border border-red-200 p-3">
                ...
                <div>
                  <span className="font-semibold text-sm text-red-800">{flag.name}</span>
                  <p className="text-sm text-text-secondary mt-0.5">{flag.reason}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {result.unrecognized_ingredients.length > 0 && (
        <div className="rounded-2xl border border-border bg-white p-6">
          <h2 className="font-display text-xl text-text mb-1">Not Scored</h2>
          <p className="text-sm text-text-muted mb-4">
